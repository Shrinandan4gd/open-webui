...
<div class="m-auto w-full max-w-6xl px-2 @2xl:px-20 translate-y-6 py-24 text-center">
	...
	>
		<div class="w-full flex flex-col justify-center items-center">
			...
			<!-- Show custom user icon if available, otherwise show default -->
			<img
			crossorigin="anonymous"
			src={$settings?.customIcon?.data ?? 
			($i18n.language === 'dg-DG'
			 ? `/doge.png`
			...
			/>
				...
				{$i18n.t('Welcome to {{name}}', { name: $settings?.assistantName?.trim() || 'Jarvis' })}
				</div>
			</div>

			<div class="flex mt-1 mb-2">
				<!-- Optional: Show assistant description here if needed in the future -->
			</div>
